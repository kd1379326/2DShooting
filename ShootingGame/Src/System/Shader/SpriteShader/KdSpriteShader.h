...
public:
	...
	//４・５月の授業用シンプル版(色変更可能版)
	void DrawColorTex(const KdTexture* tex, const Math::Rectangle& srcRect = { 0,0,0,0 }, const Math::Color& A_Color = { 0,0,0,1.0f }, const Math::Vector2& pivot = {0.5, 0.5f})
	{
		if (tex == nullptr)return;
		DrawTex(tex, 0, 0, srcRect.width, srcRect.height, &srcRect, &A_Color, pivot);
	}
